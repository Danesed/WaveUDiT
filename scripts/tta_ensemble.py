...
class Shift(_Op):
    ...
    mode="fill": voxels shifted in from outside the FOV take `fill`.  Content that
        ...
        `fill` inside its receptive field.  That is intentional — the fill value IS
    ...
    mode="roll": circular (torch.roll); bit-exact on EVERY voxel, but anatomy wraps
        ...
        useful as a zero-loss-of-support reference; "fill" is the semantically
        honest default.
    """
...
# --------------------------------------------------------------------------- #
# A view = an ordered chain of ops.                                            #
# --------------------------------------------------------------------------- #
...
def predict_tta(model, voided: torch.Tensor, mask: torch.Tensor,
    ...
    if check:
        # torch.where(mask > 0.5, ...) is only a faithful composite for a binary
        # mask; a soft mask would silently binarise. Fail loudly instead.
...
# =========================================================================== #
# Self-test.  No GPU, no data, no checkpoints: deterministic stub models whose  #
# outputs are known in closed form, so every property below is checked          #
# ALGEBRAICALLY rather than "it ran and the numbers look plausible".            #
# =========================================================================== #
def _main() -> int:  # pragma: no cover - exercised via __main__
    ...
    for s in [(0, 4, 4), (0, -4, -4), (2, -3, 5)]:
        ...
        # Analytic support: exactly ONE band of width |d| per axis is unrecoverable —
        # the trailing band for d > 0, the leading band for d < 0.
    ...
    # BlurStub commutes with flip / shift / crop in the interior, so the TTA mean of
    # correctly-inverted views must reproduce the identity prediction.
    ...
    # A shift that would amputate the brain must be refused. A POSITIVE shift drops
    # the TRAILING band, a negative one the LEADING band; test both, plus the
    # cross-case that must NOT be skipped (proves the band selection, not just "it
    # skips things").
    v3, m3 = make_case()
    ...
    print("\n=== 7. PRODUCTION shape/size: view construction + documented cost table ===")
    # No model is run here (12 forwards at 160x256x256 on CPU is pointless); this
    # validates the numbers quoted in the module docstring against build_views.
    prod = (160, 256, 256)
    ...
    # the submission path runs inside torch.autocast(bfloat16); a bf16 model output
    # must not leak into the returned dtype nor break the exact composite.
