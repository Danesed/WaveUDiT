#   https://pypi.org/project/inpainting/  (v0.0.10, Aug 2025)
#   Maintainer: Florian Kofler (TUM)
#   Source: inpainting.challenge_metrics_2023
# Used as ground-truth scorer for the BraTS 2026 Local Synthesis (Inpainting)
# challenge. Do NOT modify the metric logic — keep in sync with the package.

### FROM https://github.com/mlcommons/GaNDLF/blob/master/GANDLF/utils/tensor.py
import sys
...
### FROM https://github.com/mlcommons/GaNDLF/blob/master/GANDLF/metrics/synthesis.py
...
def _structural_similarity_index(
    ...
    """
    ssim = StructuralSimilarityIndexMeasure(return_full_image=True).to(prediction.device)
...
def _peak_signal_noise_ratio(
    ...
    if epsilon == None:
        psnr = PeakSignalNoiseRatio(data_range=data_range).to(prediction.device)
...
def _mean_squared_log_error(
    ...
    Args:
        ...
        prediction (torch.Tensor): The prediction tensor.
    """
    mle = MeanSquaredLogError().to(prediction.device)
...
def _mean_absolute_error(
    ...
    Args:
        ...
        prediction (torch.Tensor): The prediction tensor.
    """
    mae = MeanAbsoluteError().to(prediction.device)
    return mae(preds=prediction, target=target)


### FROM: https://github.com/mlcommons/GaNDLF/blob/master/GANDLF/cli/generate_metrics.py
...
def _normalize_with_percentiles(
    ...
    # Percentiles via numpy on CPU (cheap, ~50ms; identical to the original metric).
    # np.percentile cannot operate on CUDA tensors, so copy to host just for the
    # two scalars; the clip/scale below then runs on the INPUT's device so the slow
    # downstream 3D-SSIM conv can stay on GPU.
    norm_host = (
        normalization_tensor.detach().cpu().numpy()
        if torch.is_tensor(normalization_tensor)
        else normalization_tensor
    )
    v_min, v_max = np.percentile(
        norm_host, [p_min, p_max]
    ...
    if torch.is_tensor(input_tensor):
        output_tensor = torch.clamp(input_tensor, float(v_min), float(v_max))
        output_tensor = (output_tensor - float(v_min)) / (float(v_max) - float(v_min))
