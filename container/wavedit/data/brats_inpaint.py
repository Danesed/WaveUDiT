...
#
# Per the official challenge spec (see 2026_challenge-main/dataset/README.md):
...
class BraTSInpaintingDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.aug_config = dict(aug_config or {})
    ...
    def _slow_path(self, case_dir: Path) -> dict:
        ...
        dict of numpy arrays in axial-first padded space."""
        ...
        if t1_np is not None:
            t1_norm, max_v = _normalise_t1(t1_np)
            voided_norm, _ = _normalise_t1(voided_np, max_v=max_v)
        else:
    ...
    def __getitem__(self, idx):
        ...
        if (self.augment and self.mask_bank is not None
            ...
            try:
                ...
                if rng.random() < self.random_mask_prob:
                    ...
                    # tumor_mask=None: allow PERI-LESIONAL placement. The real val
                    # masks ARE peri-lesional (median ~26 vox from tumor); forcing the
                    # old ≥5-vox min-distance away from the tumor was a key cause of the
                    # train/val gap. The MaskBank pool now holds whole real masks.
                    ...
                    if new_mask is not None:
                        ...
                        # Re-derive voided from t1 and the new mask in [-1,1]
                        # normalised space. Voided sets masked region to -1
                        # (the same convention as the dataset's original
                        # voided produced by setting nifti voxels to 0 then
                        # normalising via (x/max)*2 - 1, where 0 -> -1).
        ...
        # Augmentation: rich 3D pipeline on (i_gt, i_voided, mask). Each worker
        # has its own RNG via torch's default worker seeding.
